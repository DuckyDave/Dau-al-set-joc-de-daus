...
Route::group(['prefix' => 'players'], function() {
    ...
    Route::middleware('auth:api')->group(function () {
        ...
        Route::put('/{id}', [PlayerController::class, 'update'])->name('api.players.update');
        // Un jugador específic realitza una tirada de daus
        Route::post('/{id}/games', [PlayerGameController::class, 'store'])->name('api.playerGame.store');
        // Un jugador específic llista TOTS els seus jocs (TOTES les seves tiradescde daus)
        Route::get('/{id}/games', [PlayerGameController::class, 'show'])->name('api.playerGame.show');
        // Un jugador específic elimina TOTS els seus jocs (TOTES les seves tirades de daus)
        Route::delete('/{id}/games', [PlayerGameController::class, 'destroy'])->name('api.playerGame.destroy');
    });

    // Rutes per a usuaris registats amb el rol d'administrador
    Route::middleware('auth:api')->group(function () {
        // Un adminsitrador específic crea un jugador nou
        Route::post('/', [PlayerController::class, 'store'])->name('api.players.store');
        // Un administrador específic vol veure la llista de jugadors registrats amb els seus percentages d'èxit
        Route::get('/', [PlayerRankingController::class, 'index'])->name('api.playersRanking.index');
        // Un administrador específic vol veure la llista dels jugadors registrats i el percentage mitjà d'exit
        Route::get('/ranking', [PlayerRankingController::class, 'show_average'])->name('api.playersRanking.average');
        // Un administrador específic vol veure la informació jugador amb el pitjor percentage d'exit
        Route::get('/ranking/loser', [PlayerRankingController::class, 'show_loser'])->name('api.playersRanking.loser');
        // Un administrador específic vol veure la informació del jugador amb el millor percentage d'exit
        Route::get('/ranking/winner', [PlayerRankingController::class, 'show_winner'])->name('api.playersRanking.winner');
    });
});
